...
export const ModernFeatures: React.FC = () => {
  ...
  return (
    <section id="features" className="relative py-32 px-4 overflow-hidden" style={{ perspective: '2000px' }}>
      {/* Background */}
      ...
      <div className="absolute inset-0 overflow-hidden opacity-30" style={{ transformStyle: 'preserve-3d' }}>
        <div className="absolute top-[20%] right-[10%] w-[600px] h-[600px] rounded-full bg-gradient-to-br from-orange-500/30 to-transparent blur-[120px] animate-float-slow"></div>
      ...
      <div className="container mx-auto relative z-10">
          ...
          {features.map((feature, index) => (
            ...
            >
              <CardContent className="p-8 relative z-10">
                {/* Icon with Gradient */}
                <div className={`mb-6 w-16 h-16 rounded-2xl bg-gradient-to-br ${feature.gradient} flex items-center justify-center shadow-xl shadow-orange-500/30 group-hover:shadow-orange-500/60 transition-all duration-500 group-hover:scale-125 group-hover:rotate-12 float-3d`}>
                  <feature.icon className="h-8 w-8 text-white" />
                </div>

                {/* Content */}
                <h3 className="font-display font-bold mb-3 text-2xl transition-colors duration-300 group-hover:text-primary">
                  {feature.title}
                </h3>
                <p className="text-muted-foreground font-sans transition-colors duration-300 group-hover:text-foreground/90 leading-relaxed">
                  {feature.description}
                </p>
        ...
        {/* Bottom Stats */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mt-20 stagger-animation">
          {[
            ...
            <div
              key={index}
              className="liquid-glass-intense p-6 rounded-2xl text-center group hover:scale-105 transition-all duration-500 morph-shape"
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              <stat.icon className={`h-8 w-8 mx-auto mb-3 text-${stat.color} float-3d`} />
              <div className={`text-3xl font-display font-bold bg-gradient-to-r from-orange-400 to-orange-600 bg-clip-text text-transparent mb-1`}>
                {stat.value}
              </div>
