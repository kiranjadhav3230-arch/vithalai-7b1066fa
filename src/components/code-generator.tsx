...
export const CodeGenerator = () => {
  ...
  const loadSnippets = async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('code_snippets').select('*').eq('user_id', user.id).order('created_at', {
        ascending: false
      });
  ...
  const downloadOfflineModel = async () => {
    try {
      ...
      if (usingWebGPU) {
        ...
        try {
          setDownloadProgress(0);
          const fallbackModel = await pipeline('text-generation', 'Xenova/distilgpt2', {
            device: 'wasm',
            progress_callback: (progress: any) => {
              if (progress.status === 'progress' && progress.file) {
                const percent = Math.round(progress.loaded / progress.total * 100);
  ...
  const generateCodeWithGemini = async (prompt: string, lang: string, task: string): Promise<string> => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.functions.invoke('code-generator-gemini', {
        body: {
          prompt,
          language: lang,
          task
  ...
  const downloadCode = () => {
    const extension = selectedLanguage === 'javascript' ? 'js' : selectedLanguage === 'python' ? 'py' : selectedLanguage === 'java' ? 'java' : selectedLanguage === 'cpp' ? 'cpp' : selectedLanguage === 'typescript' ? 'ts' : 'txt';
        ...
        <div className="container mx-auto p-4 md:p-6 max-w-4xl">
          <div className="mb-8 text-center space-y-4">
            <div className="flex justify-center">
              <img src={vithalLogo} alt="Vithal.AI Logo" className="h-20 w-20 object-contain" />
            </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {/* Online Model Card */}
            <Card className="relative overflow-hidden cursor-pointer hover:shadow-lg transition-all duration-300 hover:scale-105" onClick={() => setModelType('online')}>
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              <div className="space-y-4">
                ...
                {isDownloadingModel && <div className="space-y-2">
                    <div className="flex justify-between text-sm">
      ...
      <div className="container mx-auto p-4 md:p-6 max-w-7xl">
        ...
        <Tabs defaultValue="generator" className="w-full">
            ...
            <Card>
              ...
              <CardContent className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={selectedTask} onValueChange={setSelectedTask}>
                      ...
                      <SelectContent>
                        {CODE_TASKS.map(task => <SelectItem key={task.value} value={task.value}>
                ...
                <div className="space-y-2">
                  ...
                  <Textarea value={prompt} onChange={e => setPrompt(e.target.value)} placeholder="Example: Create a function that sorts an array using quicksort algorithm..." className="min-h-[140px] border-2 font-mono text-sm" />
                  <p className="text-xs text-muted-foreground">
                    Be specific about functionality, edge cases, and any special requirements
                  </p>
                </div>

                <Button onClick={generateCode} disabled={isGenerating || !prompt.trim()} className="w-full h-12 text-base font-semibold bg-gradient-to-r from-primary to-primary/90 hover:from-primary/90 hover:to-primary" size="lg">
                  {isGenerating ? <>
          ...
          <TabsContent value="snippets" className="space-y-6">
            {isLoadingSnippets ? <div className="text-center py-12">
                <Loader2 className="h-8 w-8 animate-spin mx-auto text-primary" />
                <p className="text-muted-foreground mt-4">Loading snippets...</p>
              </div> : savedSnippets.length === 0 ? <Card>
                ...
                {savedSnippets.map(snippet => <Card key={snippet.id} className="overflow-hidden border-2 hover:border-primary/30 transition-colors">
                    <CardHeader className="bg-gradient-to-r from-card to-card/50 border-b">
                      <div className="flex items-start justify-between flex-wrap gap-4">
                        ...
                        <div className="flex gap-2">
                          <Button onClick={() => copySnippetToClipboard(snippet.generated_code)} variant="outline" size="sm" className="gap-2 hover:border-primary">
                            <Copy className="h-4 w-4" />
                            Copy
                          </Button>
                          <Button onClick={() => deleteSnippet(snippet.id)} variant="outline" size="sm" className="gap-2 text-destructive hover:text-destructive hover:border-destructive">
                    ...
                    <CardContent className="p-0">
                      <ScrollArea className="h-[350px] w-full">
                        <SyntaxHighlighter language={snippet.language === 'cpp' ? 'cpp' : snippet.language} style={vscDarkPlus} showLineNumbers={true} wrapLines={true} customStyle={{
                    margin: 0,
                    borderRadius: 0,
                    fontSize: '0.875rem',
                    padding: '1.5rem',
